...
module.exports = Mixin((SuperClass) => class extends SuperClass {
    ...
    * @param {String} [defaultBlock = latest] 조회할 블록 정의
   ...
   async getBalance(unit, defaultBlock = 'latest') {

      // 주소 하나 조회
      if (this._accounts.size === 1) {
         const balance = await web3Eth.getBalance(this._web3, this.accounts[0], defaultBlock);
         const details = new Map().set(this.accounts[0], this.fromWei(balance, unit));

         this.setData(this.fromWei(balance, unit), details);
         return this.result;

      // 주소 멀티 조회
      } else if (this._accounts.size > 1) {
         const balances = await web3Eth.getBalanceBatch(this._web3, this.accounts, defaultBlock);
         let totalBalance = new BigNumber(0);
         const details = new Map();

         for (let i in balances) {
            totalBalance = totalBalance.plus(balances[i]);
            details.set(this.accounts[i], this.fromWei(balances[i], unit));
         }

         this.setData(this.fromWei(totalBalance.toFixed(), unit), details);
         return this.result;
      }
      ...
      /* // 이더스캔 주소 멀티 조회
         ...
         for (let i in balances) {
            ...
            details.set(balances[i].account, this.fromWei(balances[i].balance, unit));
         }
   ...
   async getInternalTransactions(
      ...
      const txs = await ethscanAccounts.getInternalTransactions(this._ethscan, address, sort, offset, page);
